...
public static partial class ExtensionMethods
{
  ...
  {
    if (incrementalSearch.Any(d => object.ReferenceEquals(d.Grid, grid))) return;
    ...
    grid.KeyPress += new System.Windows.Forms.KeyPressEventHandler(dataGrid_KeyPress);
  }
  ...
  /// <param name="e"></param>
  private static void dataGrid_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
  {
    var data = incrementalSearch.SingleOrDefault(d => object.ReferenceEquals(d.Grid, sender));
    if (data == null) return;
  ...
  public static int BinarySearch(this TreeNodeCollection nodes, string text)
  {
    ...
    {
      int num2 = i + (num - i >> 1);
      int num3 = string.Compare(nodes[num2].Text, text, true);
      if (num3 == 0)
      {
        return num2;
      }
  ...
  public static T SafeGetDataBoundItem<T>(this DataGridViewRow row) where T : class
  {
    ...
    catch (IndexOutOfRangeException) { retval = null; }
    return retval;
  }
  ...
  public static IEnumerable<T> GetControls<T>(this Control owner, bool subControls) where T : Control
  {
    foreach (Control control in owner.Controls)
    {
      ...
      if (subControls)
      {
        foreach (T child in control.GetControls<T>(subControls))
        {
          yield return child;
        }
      }
